...
- Version `0.0.3` targets macOS 13.0 and is packaged with an ad-hoc app signature, without Apple Developer ID notarization. Gatekeeper requires first-launch approval through right-click Open or Privacy & Security → Open Anyway. Sparkle update archives remain independently verified with EdDSA.
...
- `DEFERRED` Add Apple Developer ID signing and notarization. The release owner chose free ad-hoc distribution instead of joining the paid Apple Developer Program; revisit if first-launch Gatekeeper friction becomes unacceptable.
...
- 2026-09-02: Chose free ad-hoc distribution for `0.0.3` instead of Apple Developer ID notarization. The app and documentation must state that users need right-click Open or Privacy & Security → Open Anyway on first launch; Sparkle EdDSA verification remains required.
